...
* Compatible with `rosetta-soil` Python package v0.3.0
  * Updated `run_rosetta()` and `predict.Rosetta()` to handle the 7-parameter output (adding `K0` and `L`) introduced in `rosetta-soil` v0.3.0.
  ...
  * Added `UnsaturatedK()` R constructor and `predict.UnsaturatedK()` method for predicting `K0` and `L` from retention parameters (requires `rosetta-soil` >= 0.3.0).
  * Added `rosesoil()` R wrapper for the new upstream `rosesoil()` function (requires `rosetta-soil` >= 0.3.0).
  * Deprecated `SoilDataFromArray()` in favor of direct list input (supported in `rosetta-soil` >= 0.3.0).
  * Deprecated `ann_predict()` as the underlying Python method has been removed in v0.3.0. It now redirects to `predict()`.
